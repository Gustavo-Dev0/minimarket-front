import { Button, Icon, Theme, Typography, useMediaQuery, Box, styled, alpha, InputBase, useTheme, makeStyles } from '@mui/material';
...
export const Sales = () => {
    ...
    useEffect(() => {
      setProductsItems(top100Films)
    }, [])
    
    const handleClickSelectProduct = () => {
        if(currentProduct == null) {
            console.log("Seleccione producto")
            return
        }

        let p = productsSelected;
        let nP = [...productsSelected, currentProduct]
        setProductsSelected(nP)
    ...
    return (
        ...
        >
            <Box display='flex' flexDirection='row' gap={2} paddingLeft={2} paddingBottom={2}>
                ...
                <AutocompleteProduct items={productsItems} setValue={setCurrentProduct}/>
                <Button color='success' variant='contained' size={smDown ? 'small' : 'small'} sx={{ borderRadius: 30 }}
                    ...
                    onClick={handleClickSelectProduct} >
                    <Typography
                        variant='caption'
                    >
                        {!smDown && 'Seleccionar producto'}
                    </Typography>
                </Button>
            </Box>
            <MakeSales itemList={productsSelected}/>
        </LayoutBaseDePagina>
    );
};
